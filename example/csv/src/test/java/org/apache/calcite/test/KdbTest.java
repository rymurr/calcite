...
public class KdbTest {
  ...
  @Test public void testVanityDriver() throws SQLException {
    ...
    Connection connection =
        DriverManager.getConnection("jdbc:kdb:", info);
    connection.close();
  }
  ...
  @Test public void testVanityDriverArgsInUrl() throws SQLException {
    Connection connection =
        DriverManager.getConnection("jdbc:kdb:"
            + "directory='foo'");
    connection.close();
  }
  ...
  @Test public void testBadDirectory() throws SQLException {
    ...
    info.put("model",
        "inline:"
            ...
            + "       type: 'custom',\n"
            + "       name: 'bad',\n"
            + "       factory: 'org.apache.calcite.adapter.csv.KdbSchemaFactory',\n"
            + "       operand: {\n"
            + "         hostname: 'localhost',\n"
            + "         port: 1234\n"
            + "       }\n"
    ...
    return new Function<ResultSet, Void>() {
      public Void apply(ResultSet resultSet) {
        try {
          ...
          KdbTest.collect(lines, resultSet);
  ...
  private String resourcePath(String path) {
    final URL url = KdbTest.class.getResource("/" + path);
