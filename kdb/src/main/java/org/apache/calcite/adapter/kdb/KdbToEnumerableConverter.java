...
public class KdbToEnumerableConverter
  ...
  public Result implement(EnumerableRelImplementor implementor, Prefer pref) {
    ...
    //
    //   ((KdbTable) schema.getTable("zips")).find(
    //     "{state: 'CA'}",
    //     "{city: 1, zipcode: 1}")
    //
    //   ((KdbTable) schema.getTable("zips")).aggregate(
    //     "{$filter: {state: 'CA'}}",
    ...
    final Expression fields =
        list.append("fields",
            constantArrayList(
                Pair.zip(KdbRules.mongoFieldNames(rowType),
    ...
    Expression enumerable =
        list.append("enumerable",
            Expressions.call(table,
                KdbMethod.KDB_QUERYABLE_AGGREGATE.method, fields, ops));
